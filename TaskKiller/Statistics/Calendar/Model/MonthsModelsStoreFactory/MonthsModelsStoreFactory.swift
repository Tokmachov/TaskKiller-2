...
struct MonthsModelsStoreFactory {
    static func makeMonthModelsStore(startYear: Int, endYear: Int) -> MonthsStore {
        guard endYear > startYear else { fatalError() }
        var months = [Month]()
        for year in startYear...endYear {
            months += makeMonthsModels(year: year)
        }
        let store = MonthsStoreImp(months: months)
        return store
    }
    
    static private func makeMonthsModels(year: Int) -> [Month] {
        let monthsRange = 1...12
        var monthsModels = [Month]()
    ...
    static private func makeMonthModel(year: Int, month: Int) -> Month {
